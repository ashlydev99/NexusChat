...
import com.nexuschat.messenger.util.MailtoUtil;
  ...
  private void handleResolvedMedia(Intent intent) {
    ...
    /*
      ...
      try {
        ...
        // to EXPLICITLY fire NC's sharing activity:
        // emailIntent.setComponent(new ComponentName("com.b44t.messenger.beta", "com.nexuschat.messenger.ShareActivity"));
    ...
    */
    ...
    if (chatId == -1 && extraEmail != null && extraEmail.length > 0) {
      ...
      int contactId = ncContext.lookupContactIdByAddr(addr);

      if (contactId == 0) {
        contactId = ncContext.createContact(null, addr);
      }

      chatId = ncContext.createChatByContactId(contactId);
    ...
    if (accId != -1 && chatId > 0) {
      ...
      ShareUtil.setSharedUris(composeIntent, resolvedExtras);
      ShareUtil.setIsFromWebxnc(composeIntent, ShareUtil.isFromWebxnc(this));
