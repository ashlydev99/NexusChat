...
import com.nexuschat.messenger.ConversationListItem;
...
class SearchListAdapter
  ...
  final NcContext ncContext; // reset on account switching is not needed because SearchFragment and
  ...
  public void onBindViewHolder(@NonNull SearchResultViewHolder holder, int position) {
    NcChatlist.Item conversationResult = getConversationResult(position);
    ...
    NcContact contactResult = getContactResult(position);
    ...
    NcMsg messageResult = getMessageResult(position);
  ...
  @Nullable
  private NcChatlist.Item getConversationResult(int position) {
  ...
  @Nullable
  private NcContact getContactResult(int position) {
    if (position >= getFirstContactIndex() && position < getFirstMessageIndex()) {
      return ncContext.getContact(searchResult.getContacts()[position - getFirstContactIndex()]);
    }
    return null;
  }

  @Nullable
  private NcMsg getMessageResult(int position) {
    if (position >= getFirstMessageIndex() && position < searchResult.size()) {
      return ncContext.getMsg(searchResult.getMessages()[position - getFirstMessageIndex()]);
    }
    return null;
  }
  ...
  public interface EventListener {
    void onConversationClicked(@NonNull NcChatlist.Item chatlistItem);

    void onConversationLongClicked(@NonNull NcChatlist.Item chatlistItem);

    void onContactClicked(@NonNull NcContact contact);

    void onMessageClicked(@NonNull NcMsg message);
  }
  ...
  static class SearchResultViewHolder extends RecyclerView.ViewHolder {
    ...
    void bind(
        Context context,
        @NonNull NcChatlist.Item chatlistItem,
      ...
      ThreadRecord threadRecord =
          NcHelper.getThreadRecord(
              context, chatlistItem.summary, ncContext.getChat(chatlistItem.chatId));
      root.bind(
          threadRecord,
          chatlistItem.msgId,
    ...
    void bind(
        @NonNull NcContact contactResult,
        @NonNull GlideRequests glideRequests,
        @NonNull EventListener eventListener,
        @Nullable String query) {
