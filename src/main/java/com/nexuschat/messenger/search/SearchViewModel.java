...
class SearchViewModel extends ViewModel {
  ...
  SearchViewModel(@NonNull Context context) {
    this.ncContext = NcHelper.getContext(context.getApplicationContext());
  ...
  private void queryAndCallback(@NonNull String query, @NonNull SearchViewModel.Callback callback) {
    ...
    NcChatlist conversations =
        ncContext.getChatlist(forwarding ? NcContext.NC_GCL_FOR_FORWARDING : 0, query, 0);
    ...
    int[] contacts = ncContext.getContacts(NcContext.NC_GCL_ADD_SELF, query);
    ...
    startMs = System.currentTimeMillis();
    int[] messages = ncContext.searchMsgs(0, query);
