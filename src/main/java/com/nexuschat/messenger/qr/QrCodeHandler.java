...
import com.nexuschat.messenger.ConversationActivity;
...
import com.nexuschat.messenger.relay.RelayListActivity;
...
public class QrCodeHandler {
  ...
  public QrCodeHandler(Activity activity) {
    ...
    ncContext = NcHelper.getContext(activity);
    rpc = NcHelper.getRpc(activity);
    accId = ncContext.getAccountId();
  }
  ...
  public void handleOnlySecureJoinQr(
    ...
    final NcLot qrParsed = ncContext.checkQr(rawString);
  ...
  private boolean handleSecureJoinQr(
      NcLot qrParsed, String rawString, SecurejoinSource source, SecurejoinUiPath uiPath) {
    AlertDialog.Builder builder = new AlertDialog.Builder(activity);
    String name = ncContext.getContact(qrParsed.getId()).getDisplayName();
    switch (qrParsed.getState()) {
      case NcContext.NC_QR_ASK_VERIFYCONTACT:
      case NcContext.NC_QR_ASK_VERIFYGROUP:
      case NcContext.NC_QR_ASK_JOIN_BROANCAST:
        showVerifyContactOrGroup(builder, rawString, qrParsed, name, source, uiPath);
        break;

      case NcContext.NC_QR_WITHDRAW_VERIFYCONTACT:
      case NcContext.NC_QR_WITHDRAW_VERIFYGROUP:
      case NcContext.NC_QR_WITHDRAW_JOINBROANCAST:
        String message =
            qrParsed.getState() == NcContext.NC_QR_WITHDRAW_VERIFYCONTACT
                ...
                : qrParsed.getState() == NcContext.NC_QR_WITHDRAW_VERIFYCONTACT
                    ...
                    : activity.getString(
                        R.string.withdraw_joinbroancast_explain, qrParsed.getText1());
        builder.setTitle(R.string.qrshow_title);
        builder.setMessage(message);
        builder.setNeutralButton(
            R.string.reset,
            (dialog, which) -> {
              ncContext.setConfigFromQr(rawString);
            });
        builder.setPositiveButton(R.string.ok, null);
        Util.redButton(builder.show(), AlertDialog.BUTTON_NEUTRAL);
        return true;

      case NcContext.NC_QR_REVIVE_VERIFYCONTACT:
      case NcContext.NC_QR_REVIVE_VERIFYGROUP:
      case NcContext.NC_QR_REVIVE_JOINBROANCAST:
        builder.setTitle(R.string.qrshow_title);
      ...
      case NcContext.NC_QR_FPR_WITHOUT_ADDR:
        showVerifyFingerprintWithoutAddress(builder, qrParsed);
        break;

      case NcContext.NC_QR_FPR_MISMATCH:
        showFingerPrintError(builder, name);
        break;

      case NcContext.NC_QR_FPR_OK:
      case NcContext.NC_QR_ADDR:
  ...
  /** Process only QR about adding relays/profiles (NCACCOUNT: / NCLOGIN:) */
  ...
  private boolean handleAddRelayQr(
    ...
    switch (qrParsed.getState()) {
      case NcContext.NC_QR_ACCOUNT:
      case NcContext.NC_QR_LOGIN:
        AlertDialog.Builder builder = new AlertDialog.Builder(activity);
  ...
  public boolean handleProxyQr(String rawString) {
    return handleProxyQr(ncContext.checkQr(rawString), rawString);
  }

  private boolean handleProxyQr(NcLot qrParsed, String rawString) {
  ...
  public boolean handleBackupQr(String rawString) {
    return handleBackupQr(ncContext.checkQr(rawString), rawString);
  }

  private boolean handleBackupQr(NcLot qrParsed, String rawString) {
    switch (qrParsed.getState()) {
      case NcContext.NC_QR_BACKUP2:
        AlertDialog.Builder builder = new AlertDialog.Builder(activity);
      ...
      case NcContext.NC_QR_BACKUP_TOO_NEW:
  ...
  public void handleQrData(
    ...
    AlertDialog.Builder builder = new AlertDialog.Builder(activity);
    if (qrParsed.getState() == NcContext.NC_QR_URL) {
  ...
  private void handleDefault(AlertDialog.Builder builder, String qrRawString, NcLot qrParsed) {
    String msg;
    final String scannedText;
    switch (qrParsed.getState()) {
      case NcContext.NC_QR_ERROR:
  ...
  private void showQrUrl(AlertDialog.Builder builder, NcLot qrParsed) {
  ...
  private void showFingerprintOrQrSuccess(
      AlertDialog.Builder builder, NcLot qrParsed, String name) {
    @StringRes
    int resId =
        qrParsed.getState() == NcContext.NC_QR_ADDR
    ...
    builder.setPositiveButton(
        R.string.start_chat,
        (dialogInterface, i) -> {
          int chatId = ncContext.createChatByContactId(qrParsed.getId());
          Intent intent = new Intent(activity, ConversationActivity.class);
          intent.putExtra(ConversationActivity.CHAT_ID_EXTRA, chatId);
          if (qrParsed.getText1Meaning() == NcLot.NC_TEXT1_DRAFT) {
  ...
  private void showVerifyFingerprintWithoutAddress(AlertDialog.Builder builder, NcLot qrParsed) {
    builder.setMessage(
  ...
  public void secureJoinByQr(String qrRawString, SecurejoinSource source, SecurejoinUiPath uipath) {
    try {
      int newChatId =
          NcHelper.getRpc(activity)
              .secureJoinWithUxInfo(ncContext.getAccountId(), qrRawString, source, uipath);
