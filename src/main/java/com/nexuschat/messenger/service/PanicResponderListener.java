package com.nexuschat.messenger.service;
 ...
 * creating "panic button" apps that can trigger actions in "panic responder" apps. In this case,
 * the response is to lock the app, if it has been configured to do so via the Signal lock
 * preference. If the user has not set a passphrase, then the panic trigger intent does nothing.
 */
public class PanicResponderListener extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    if (intent != null
        && !Prefs.isPasswordDisabled(context)
