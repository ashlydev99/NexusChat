...
public class LongClickCopySpan extends ClickableSpan {
  ...
  private void openChat(Activity activity, NcContact contact) {
    NcContext ncContext = NcHelper.getContext(activity);
    int chatId = ncContext.createChatByContactId(contact.getId());
    if (chatId != 0) {
  ...
  public void onClick(View widget) {
    if (url.startsWith(PREFIX_CMD)) {
      ...
      try {
        ...
        if (contactId == 0 && ncContext.mayBeValidAddr(addr)) {
          contactId = ncContext.createContact(null, addr);
        }
        NcContact contact = ncContext.getContact(contactId);
        if (contact.getId() != 0
            && !contact.isBlocked()
