...
import static com.nexuschat.messenger.util.ShareUtil.getForwardedMessageIDs;
...
import com.nexuschat.messenger.ConversationListRelayingActivity;
import com.nexuschat.messenger.connect.NcHelper;
import com.nexuschat.messenger.mms.PartAuthority;
...
public class SendRelayedMessageUtil {
  ...
  public static void immediatelyRelay(Activity activity, final Long[] chatIds) {
    ...
    if (isForwarding(activity)) {
      ...
      Util.runOnAnyBackgroundThread(
          () -> {
            NcContext ncContext = DcHelper.getContext(activity);
  ...
  private static void handleForwarding(Context context, int chatId, int[] forwardedMessageIDs) {
    ...
    ncContext.forwardMsgs(forwardedMessageIDs, chatId);
  }
  ...
  public static void sendMultipleMsgs(
    ...
    if (uris.size() == 1) {
      ncContext.sendMsg(chatId, createMessage(context, uris.get(0), text));
    } else {
      if (text != null) {
        ncContext.sendMsg(chatId, createMessage(context, null, text));
      }
      for (Uri uri : uris) {
        ncContext.sendMsg(chatId, createMessage(context, uri, null));
      }
    }
  }
  ...
  public static NcMsg createMessage(Context context, Uri uri, String text)
      throws NullPointerException {
    NcContext ncContext = NcHelper.getContext(context);
    NcMsg message;
    String mimeType = MediaUtil.getMimeType(context, uri);
    if (uri == null) {
      message = new NcMsg(ncContext, NcMsg.NC_MSG_TEXT);
    } else if (MediaUtil.isImageType(mimeType)) {
      message = new NcMsg(ncContext, NcMsg.NC_MSG_IMAGE);
    } else if (MediaUtil.isAudioType(mimeType)) {
      message = new NcMsg(ncContext, NcMsg.NC_MSG_AUDIO);
    } else if (MediaUtil.isVideoType(mimeType)) {
      message = new NcMsg(ncContext, NcMsg.NC_MSG_VIDEO);
    } else {
      message = new NcMsg(ncContext, NcMsg.NC_MSG_FILE);
    }
  ...
  private static void setFileFromUri(Context context, Uri uri, DcMsg message, String mimeType) {
    ...
    try {
      ...
      path = NcHelper.getBlobdirFile(ncContext, filename, "temp");
