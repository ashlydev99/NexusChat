...
/**
 * An image view that maintains the size of the drawable provided. That means: when you set an image
 * drawable it will be scaled to fit the screen once. If you rotate the screen it will be rescaled
 * to fit. If you crop the screen (e.g. because the soft keyboard is displayed) the image is cropped
 * instead.
 *
 * @author Angelo Fuchs
 */
public class ScaleStableImageView extends AppCompatImageView
  ...
  protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
    ...
    if (keyboardShown) {
      // don't scale; Crop.
      Drawable large;
      if (portrait) large = storedSizes.get(portraitWidth + "x" + portraitHeight);
      else large = storedSizes.get(landscapeWidth + "x" + landscapeHeight);
  ...
  private void measureViewSize(
      int width, int height, int oldWidth, int oldHeight, boolean portrait) {
