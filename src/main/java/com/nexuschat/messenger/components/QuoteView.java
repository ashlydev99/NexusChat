...
import com.nexuschat.messenger.recipients.RecipientForeverObserver;
...
public class QuoteView extends FrameLayout implements RecipientForeverObserver {
  ...
  private NcMsg quotedMsg;
  private NcContact author;
  ...
  public void setQuote(
      ...
      boolean isEdit) {
    quotedMsg = msg;
    this.author = author != null ? author.getNcContact() : null;
    this.body = body;
  ...
  private void setQuoteAuthor(@Nullable Recipient author) {
    if (isEdit) {
      ...
      NcContact contact = author.getNcContact();
      authorView.setVisibility(VISIBLE);
      if (contact == null) {
      ...
      if (slide.isWebxdcDocument()) {
        ...
        try {
          VcardContact vcardContact =
              NcHelper.getRpc(getContext()).parseVcard(quotedMsg.getFile()).get(0);
  ...
  public NcContact getDcContact() {
    return author;
  }

  public NcMsg getOriginalMsg() {
    return quotedMsg;
  }
