package com.nexuschat.messenger.components.reminder;
...
public class DozeReminder {
  ...
  public static boolean isEligible(Context context) {
    ...
    try {
      int numberOfChats = NcHelper.getContext(context).getChatlist(0, null, 0).getCnt();
      if (numberOfChats <= 2) {
        return false;
      }
    } catch (Exception e) {
  ...
  public static void addDozeReminderDeviceMsg(Context context) {
    ...
    int msgId = ncContext.addDeviceMsg("android.doze-reminder", msg);
    if (msgId != 0) {
      Prefs.setPromptedDozeMsgId(context, msgId);
    }
  }

  public static boolean isDozeReminderMsg(Context context, NcMsg msg) {
    return msg != null
