...
public class AttachmentsContentProvider extends ContentProvider {
  ...
  public ParcelFileDescriptor openFile(Uri uri, @NonNull String mode) throws FileNotFoundException {
    ...
    if (!NcHelper.sharedFiles.containsKey(file)) {
      throw new FileNotFoundException("File was not shared before.");
    }

    File privateFile = new File(ncContext.getBlobdir(), file);
  ...
  public String getType(Uri uri) {
    String file = uri.getPathSegments().get(0);
    String mimeType = NcHelper.sharedFiles.get(file);

    return NcHelper.checkMime(uri.getPathSegments().get(1), mimeType);
  }
  ...
  public Cursor query(
    ...
    NcContext ncContext = NcHelper.getContext(Objects.requireNonNull(getContext()));
    File privateFile = new File(ncContext.getBlobdir(), file);
