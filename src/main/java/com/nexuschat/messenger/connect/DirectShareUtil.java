  ...
  private static List<ShortcutInfoCompat> getChooserTargets(Context context) {
    ...
    NcChatlist chatlist =
        dcContext.getChatlist(
            NcContext.NC_GCL_FOR_FORWARDING | NcContext.NC_GCL_NO_SPECIALS, null, 0);
    int max = 5;
    if (chatlist.getCnt() < max) {
      max = chatlist.getCnt();
    }
    for (int i = 0; i < max; i++) {
      NcChat chat = chatlist.getChat(i);
      if (!chat.canSend()) {
        continue;
      }
      ...
      results.add(
          new ShortcutInfoCompat.Builder(
                  context, "chat-" + ncContext.getAccountId() + "-" + chat.getId())
              ...
              .setIntent(intent)
              .setActivity(new ComponentName(context, "com.nexuschat.messenger.RoutingActivity"))
              .build());
    }
