...
final class Bisect {
  ...
  /**
   * Given a predicate function, attempts to finds the boundary between predicate true and predicate
   * false. If it returns true, it will animate the element to the closest true value found to that
   * boundary.
   *
   * @param element The element to modify.
   * @param outOfBoundsValue The current value, known to be out of bounds. 1 for a scale and 0 for a
   *     translate.
  ...
  static boolean bisectToTest(
    ...
    Matrix closestSuccesful =
        bisectToTest(element, outOfBoundsValue, atMost, predicate, modifyElement);
  ...
  /**
   * Given a predicate function, attempts to finds the boundary between predicate true and predicate
   * false. Returns new local matrix for the element if a solution is found.
   *
   * @param element The element to modify.
   * @param outOfBoundsValue The current value, known to be out of bounds. 1 for a scale and 0 for a
   *     translate.
   ...
   */
  static @Nullable Matrix bisectToTest(
      @NonNull EditorElement element,
      float outOfBoundsValue,
      float atMost,
      @NonNull Predicate predicate,
    ...
    do {
      ...
      try {
        ...
        if (predicate.test()) {
          ...
          if (!haveResult
              || Math.abs(nextValueToTry - outOfBoundsValue)
                  < Math.abs(successValue - outOfBoundsValue)) {
            haveResult = true;
          ...
          if (attempt == 1) {
            // failure on first attempt means inBoundsValue is actually out of bounds and so no
            // solution
            return null;
          }
