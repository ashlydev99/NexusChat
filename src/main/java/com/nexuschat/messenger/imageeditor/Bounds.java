...
/**
 * The local extent of a {@link com.nexuschat.messenger.imageeditor.model.EditorElement}. i.e.
 * all {@link com.nexuschat.messenger.imageeditor.model.EditorElement}s have a bounding rectangle
 * from:
 *
 * <p>{@link #LEFT} to {@link #RIGHT} and from {@link #TOP} to {@link #BOTTOM}.
 */
public final class Bounds {
  ...
  public static boolean contains(float x, float y) {
    return x >= FULL_BOUNDS.left
        && x <= FULL_BOUNDS.right
        && y >= FULL_BOUNDS.top
        && y <= FULL_BOUNDS.bottom;
  }

  /**
   * Maps all the points of bounds with the supplied matrix and determines whether they are still in
   * bounds.
   *
