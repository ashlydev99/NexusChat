package com.nexuschat.messenger.calls;

import static com.nexuschat.messenger.calls.CallUtil.getIconFromChat;
import static com.nexuschat.messenger.calls.CallUtil.getNameFromChat;
...
public class CallCoordinator implements NcEventCenter.NcEventDelegate {
  ...
  private CallCoordinator(Context context) {
    ...
    this.rpc = NcHelper.getRpc(this.appContext);
  ...
  private void createNotificationChannels() {
    ...
    incomingChannel.setDescription("Notifications for incoming NexusChat calls");
    ...
    ongoingChannel.setDescription("Notifications for active NexusChat calls");
  ...
  private void addEventListeners() {
    NcEventCenter eventCenter = NcHelper.getEventCenter(this.appContext);
    eventCenter.removeObservers(this);

    eventCenter.addMultiAccountObserver(NcContext.NC_EVENT_INCOMING_CALL, this);
    eventCenter.addMultiAccountObserver(NcContext.NC_EVENT_INCOMING_CALL_ACCEPTED, this);
    eventCenter.addMultiAccountObserver(NcContext.NC_EVENT_OUTGOING_CALL_ACCEPTED, this);
    eventCenter.addMultiAccountObserver(NcContext.NC_EVENT_CALL_ENDED, this);
  }
  ...
  public void handleEvent(@NonNull NcEvent event) {
    ...
    eventExecutor.execute(
        () -> {
          ...
          switch (eventId) {
            case NcContext.NC_EVENT_INCOMING_CALL:
              try {
  ...
  private synchronized void onIncomingCall(
    ...
    // Get caller info
    NcContext ncContext = ApplicationContext.getNcAccounts().getAccount(accId);
    int chatId = ncContext.getMsg(callId).getChatId();
    this.activeChatId = chatId;
    NcChat ncChat = ncContext.getChat(chatId);
    String callerName = getNameFromChat(ncChat);
    Icon callerIcon = getIconFromChat(this.appContext, ncChat);
  ...
  public synchronized void initiateOutgoingCall(int accId, int chatId, boolean startsWithVideo) {
    ...
    NcContext ncContext = ApplicationContext.getNcAccounts().getAccount(accId);
    NcChat ncChat = ncContext.getChat(chatId);
    String calleeName = getNameFromChat(ncChat);
    displayName.postValue(calleeName);

    // This is fine because icon is UI only and not used elsewhere
    new Thread(
            () -> {
              Icon calleeIcon = getIconFromChat(this.appContext, ncChat);
