...
import com.nexuschat.messenger.components.AvatarSelector;
...
import com.nexuschat.messenger.qr.RegistrationQrActivity;
...
import com.nexuschat.messenger.util.TextUtil;
...
public class InstantOnboardingActivity extends BaseActionBarActivity
  ...
  private static final String NCACCOUNT = "ncaccount";
  private static final String NCLOGIN = "nclogin";
  ...
  private NcLot parsedQrData;
  private boolean isNcLogin;
  ...
  public void onCreate(Bundle bundle) {
    ...
    boolean configured = NcHelper.getContext(this).isConfigured() == 1;

    if (configured) {
      // if account is configured it means we didn't come from Welcome screen nor from QR scanner,
      // instead, user clicked a ncaccount:// URI directly, so we need to just offer to add a new
      // relay
    ...
    providerQrData = NCACCOUNT + ":" + providerHost;
  ...
  private void setProviderFromQr(String rawQr) {
    NcLot qrParsed = ncContext.checkQr(rawQr);
    switch (qrParsed.getState()) {
      case NcContext.NC_QR_LOGIN:
        isNcLogin = true; // Intentional fall-through
  ...
  protected void onPause() {
    ...
    if (NcHelper.getContext(this).isConfigured() == 0) {
      final String displayName = name.getText().toString();
      NcHelper.set(
          this, NcHelper.CONFIG_DISPLAY_NAME, TextUtils.isEmpty(displayName) ? null : displayName);
  ...
  private void handleIntent() {
    if (getIntent() != null && Intent.ACTION_VIEW.equals(getIntent().getAction())) {
      ...
      if (uri.getScheme().equalsIgnoreCase(NCACCOUNT)
          || uri.getScheme().equalsIgnoreCase(NCLOGIN)) {
    ...
    if (isNcLogin) {
      ...
      if (parsedQrData != null) {
        if (isContactInvitation) {
          String name = ncContext.getContact(parsedQrData.getId()).getDisplayName();
  ...
  private void initializeProfile() {
    ...
    name.setText(NcHelper.get(this, NcHelper.CONFIG_DISPLAY_NAME));
  }
  ...
  private void createProfile() {
    ...
    executor.execute(
        () -> {
          ...
          NcHelper.set(context, NcHelper.CONFIG_DISPLAY_NAME, name);
  ...
  private void startQrAccountCreation(String qrCode) {
    ...
    NcHelper.getEventCenter(this).captureNextError();

    new Thread(
            () -> {
              Rpc rpc = NcHelper.getRpc(this);
              try {
                rpc.addTransportFromQr(ncContext.getAccountId(), qrCode);
                NcHelper.getEventCenter(this).endCaptureNextError();
                progressSuccess();
