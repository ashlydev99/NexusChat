...
public class AddReactionView extends LinearLayout {
  ...
  public void show(NcMsg msgToReactTo, View parentView, AddReactionListener listener) {
    init(); // init delayed as needed

    if (msgToReactTo.isInfo() || !ncContext.getChat(msgToReactTo.getChatId()).canSend()) {
      return;
    }
  ...
  private String getSelfReaction() {
    ...
    try {
      final Reactions reactions =
          rpc.getMessageReactions(ncContext.getAccountId(), msgToReactTo.getId());
      if (reactions != null) {
  ...
  private void sendReaction(final String reaction) {
    try {
      if (reaction == null || reaction.equals(getSelfReaction())) {
        rpc.sendReaction(
            ncContext.getAccountId(), msgToReactTo.getId(), Collections.singletonList(""));
      } else {
        rpc.sendReaction(
            ncContext.getAccountId(), msgToReactTo.getId(), Collections.singletonList(reaction));
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
