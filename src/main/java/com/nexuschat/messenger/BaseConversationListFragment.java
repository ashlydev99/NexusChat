...
import static com.nexuschat.messenger.util.ShareUtil.getSharedUris;
...
import com.nexuschat.messenger.util.task.SnackbarAsyncTask;
...
public abstract class BaseConversationListFragment extends Fragment implements ActionMode.Callback {
  ...
  private boolean areSomeSelectedChatsUnpinned() {
    ...
    for (long chatId : selectedChats) {
      ...
      if (ncChat.getVisibility() != NcChat.NC_CHAT_VISIBILITY_PINNED) {
        return true;
      }
    }
    return false;
  ...
  private void handlePinAllSelected() {
    final NcContext ncContext = NcHelper.getContext(requireActivity());
    ...
    for (long chatId : selectedConversations) {
      ncContext.setChatVisibility(
          (int) chatId,
          doPin ? NcChat.NC_CHAT_VISIBILITY_PINNED : NcChat.NC_CHAT_VISIBILITY_NORMAL);
    }
    if (actionMode != null) {
      actionMode.finish();
      actionMode = null;
  ...
  private void handleMuteAllSelected() {
    ...
    if (areSomeSelectedChatsUnmuted()) {
      MuteDialog.show(
          ...
          duration -> {
            for (long chatId : selectedConversations) {
              ncContext.setChatMuteDuration((int) chatId, duration);
            }
      ...
      // unmute
      for (long chatId : selectedConversations) {
        ncContext.setChatMuteDuration((int) chatId, 0);
      }
  ...
  private void handleMarknoticedSelected() {
    ...
    final Set<Long> selectedConversations =
        new HashSet<Long>(getListAdapter().getBatchSelections());
    for (long chatId : selectedConversations) {
      ncContext.marknoticedChat((int) chatId);
    }
    if (actionMode != null) {
      actionMode.finish();
      actionMode = null;
    ...
    new SnackbarAsyncTask<Void>(
      ...
      protected void executeAction(@Nullable Void parameter) {
        for (long chatId : selectedConversations) {
          ncContext.setChatVisibility(
              (int) chatId,
              archive ? NcChat.NC_CHAT_VISIBILITY_ARCHIVED : NcChat.NC_CHAT_VISIBILITY_NORMAL);
        }
      }
      ...
      protected void reverseAction(@Nullable Void parameter) {
        for (long threadId : selectedConversations) {
          ncContext.setChatVisibility(
              (int) threadId,
              archive ? NcChat.NC_CHAT_VISIBILITY_NORMAL : NcChat.NC_CHAT_VISIBILITY_ARCHIVED);
        }
      }
  ...
  private void handleDeleteAllSelected() {
    ...
    if (chatsCount == 1) {
      ...
      alertText =
          activity
              .getResources()
              .getString(R.string.ask_delete_named_chat, ncContext.getChat((int) chatId).getName());
    } else {
      alertText =
          activity
              .getResources()
    ...
    for (long chatId : selectedChats) {
      if (ncContext.getChat((int) chatId).shallLeaveBeforeDelete(ncContext)) {
          ...
          if (!selectedChats.isEmpty()) {
            new AsyncTask<Void, Void, Void>() {
              ...
              protected Void doInBackground(Void... params) {
                ...
                for (long chatId : selectedChats) {
                  NcHelper.getNotificationCenter(requireContext())
                      .removeNotifications(accountId, (int) chatId);
                  if (ncContext.getChat((int) chatId).shallLeaveBeforeDelete(ncContext)) {
                    ncContext.removeContactFromChat((int) chatId, NcContact.NC_CONTACT_ID_SELF);
                  }
                  ncContext.deleteChat((int) chatId);
  ...
  private void handleAddToHomeScreen() {
    ...
    final NcContext ncContext = NcHelper.getContext(activity);
    final Set<Long> selectedChats = getListAdapter().getBatchSelections();
    final NcChat chat = ncContext.getChat(selectedChats.iterator().next().intValue());

    Intent intent = new Intent(activity, ShareActivity.class);
    intent.setAction(Intent.ACTION_SEND);
    intent.putExtra(ShareActivity.EXTRA_ACC_ID, ncContext.getAccountId());
        ...
        () -> {
          ...
          ShortcutInfoCompat shortcutInfoCompat =
              new ShortcutInfoCompat.Builder(
                      activity, "chat-" + ncContext.getAccountId() + "-" + chat.getId())
