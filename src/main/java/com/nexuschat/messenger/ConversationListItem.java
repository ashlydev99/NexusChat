...
import com.nexuschat.messenger.components.FromTextView;
...
public class ConversationListItem extends RelativeLayout
  ...
  public void bind(
      ...
      boolean batchMode) {
    bind(thread, msgId, ncSummary, glideRequests, selectedThreads, batchMode, null);
  }
  ...
  public void bind(
    ...
    int state = ncSummary.getState();

    // CAMBIO #1: Modificar nombre del chat "Mensajes guardados" para mostrar "Nombre (Yo)"
    String displayName = recipient.getName();
    NcContext ncContext = NcHelper.getContext(getContext());
    
    // Verificar si es el chat "Mensajes guardados" (Self Talk)
    if (chatId == ncContext.getChatIdByContactId(NcContact.NC_CONTACT_ID_SELF)) {
      // Obtener el nombre del usuario actual
      NcContact selfContact = ncContext.getContact(NcContact.NC_CONTACT_ID_SELF);
      String selfName = selfContact.getDisplayName();
      if (TextUtils.isEmpty(selfName)) {
        selfName = selfContact.getAddr();
      }
      // Mostrar como "Nombre (Yo)"
      displayName = selfName + " (Yo)";
    }

    if (highlightSubstring != null) {
      this.fromView.setText(getHighlightedSpan(displayName, highlightSubstring));
    } else {
      this.fromView.setText(displayName, state != NcMsg.NC_STATE_IN_FRESH);
    }

    subjectView.setVisibility(chatId == NcChat.NC_CHAT_ID_ARCHIVED_LINK ? GONE : VISIBLE);
    this.subjectView.setText(thread.getDisplayBody());
    this.subjectView.setTypeface(state == NcMsg.NC_STATE_IN_FRESH ? BOLD_TYPEFACE : LIGHT_TYPEFACE);
    this.subjectView.setTextColor(
        state == NcMsg.NC_STATE_IN_FRESH
    ...
    dateView.setCompoundDrawablesWithIntrinsicBounds(
        ...
        thread.getVisibility() == NcChat.NC_CHAT_VISIBILITY_PINNED
    ...
    // CAMBIO #1: Para el chat "Mensajes guardados", usar la foto de perfil del usuario
    if (chatId == ncContext.getChatIdByContactId(NcContact.NC_CONTACT_ID_SELF)) {
      NcContact selfContact = ncContext.getContact(NcContact.NC_CONTACT_ID_SELF);
      Recipient selfRecipient = new Recipient(getContext(), selfContact);
      this.avatar.setAvatar(glideRequests, selfRecipient, false);
      this.avatar.setSeenRecently(selfContact.wasSeenRecently());
    } else {
  ...
  public void bind(
      @NonNull NcContact contact,
      @NonNull GlideRequests glideRequests,
      @Nullable String highlightSubstring) {
    this.selectedThreads = Collections.emptySet();
    
    // CAMBIO #1: Para el contacto propio, mostrar "Nombre (Yo)"
    String displayName = contact.getDisplayName();
    NcContext ncContext = NcHelper.getContext(getContext());
    if (contact.getId() == NcContact.NC_CONTACT_ID_SELF) {
      String selfName = displayName;
      if (TextUtils.isEmpty(selfName)) {
        selfName = contact.getAddr();
      }
      displayName = selfName + " (Yo)";
    }
    ...
    fromView.setText(getHighlightedSpan(displayName, highlightSubstring));
  ...
  public void bind(
      @NonNull NcMsg messageResult,
      @NonNull GlideRequests glideRequests,
      @Nullable String highlightSubstring) {
    NcContext ncContext = NcHelper.getContext(getContext());
    NcContact sender = ncContext.getContact(messageResult.getFromId());
    this.selectedThreads = Collections.emptySet();
  ...
  private void setStatusIcons(ThreadRecord thread, int state) {
    if (thread.getVisibility() == NcChat.NC_CHAT_VISIBILITY_ARCHIVED) {
      ...
      if (state == NcMsg.NC_STATE_OUT_FAILED) {
        deliveryStatusIndicator.setFailed();
      } else if (state == NcMsg.NC_STATE_OUT_PREPARING) {
        deliveryStatusIndicator.setPreparing();
      } else if (state == NcMsg.NC_STATE_OUT_PENDING) {
        deliveryStatusIndicator.setPending();
      } else if (thread.isOutChannel()) {
        deliveryStatusIndicator.setNone();
      } else if (state == NcMsg.NC_STATE_OUT_MDN_RCVD) {
        deliveryStatusIndicator.setRead();
      } else if (state == NcMsg.NC_STATE_OUT_DELIVERED) {
    ...
    if (unreadCount == 0 || thread.isContactRequest()) {
      ...
      boolean isMuted = thread.isMuted() || chatId == NcChat.NC_CHAT_ID_ARCHIVED_LINK;
      final int color =
          getResources()
              .getColor(
                  isMuted
  ...
  private void setBgColor(ThreadRecord thread) {
    ...
    if (thread != null && thread.getVisibility() == NcChat.NC_CHAT_VISIBILITY_PINNED) {
