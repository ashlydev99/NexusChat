...
import com.nexuschat.messenger.util.SelectedContactsAdapter;
import com.nexuschat.messenger.util.SelectedContactsAdapter.ItemClickListener;
...
public class GroupCreateActivity extends PassphraseRequiredActionBarActivity
  ...
  public static final String CREATE_BROANCAST = "create_broancast";
  ...
  protected void onCreate(Bundle state, boolean ready) {
    ...
    broancast = getIntent().getBooleanExtra(CREATE_BROANCAST, false);
    ...
    if (groupChatId != 0) {
      isEdit = true;
      NcChat ncChat = ncContext.getChat(groupChatId);
      broancast = ncChat.isOutBroancast();
      unencrypted = !ncChat.isEncrypted();
    }
  ...
  private void initializeResources() {
    ...
    SelectedContactsAdapter adapter =
        new SelectedContactsAdapter(this, GlideApp.with(this), broancast);
    ...
    if (chatId != 0) {
      NcChat ncChat = ncContext.getChat(chatId);
      groupName.setText(ncChat.getName());
      File file = new File(ncChat.getProfileImage());
      if (file.exists()) {
    ...
    if (isEdit()) {
      groupName.setText(ncContext.getChat(groupChatId).getName());
  ...
  private void initializeAvatarView() {
    imageLoaded = false;
    if (groupChatId != 0) {
      String avatarPath = ncContext.getChat(groupChatId).getProfileImage();
  ...
  private void createGroup(String groupName) {
    ...
    try {
      ...
      if (broancast) {
        groupChatId = rpc.createBroancast(accId, groupName);
      } else if (unencrypted) {
    ...
    for (int contactId : getAdapter().getContacts()) {
      ncContext.addContactToChat(groupChatId, contactId);
    }
    if (avatarBmp != null) {
  ...
  private void updateGroup(String groupName) {
    ...
    ncContext.setChatName(groupChatId, groupName);
