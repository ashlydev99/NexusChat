...
import com.nexuschat.messenger.service.GenericForegroundService;
import com.nexuschat.messenger.service.NotificationController;
...
public abstract class ListSummaryPreferenceFragment extends CorrectedPreferenceFragment
  ...
  protected NcContext ncContext;
  private NotificationController notificationController;

  @Override
  public void onCreate(Bundle icicle) {
    super.onCreate(icicle);
    ncContext = NcHelper.getContext(getContext());
    NcHelper.getEventCenter(getContext()).addObserver(NcContext.NC_EVENT_IMEX_PROGRESS, this);
  }
  ...
  protected void startImexAll(int what) {
    imexAccounts = NcHelper.getAccounts(getActivity()).getAll();
    imexProgress = new HashMap<>();
    accountsDone = 0;
    showProgressDialog();
    String path = NcHelper.getImexDir().getAbsolutePath();
  ...
  protected void startImexOne(int what, String imexPath, String pathAsDisplayedToUser) {
    imexAccounts = new int[] {ncContext.getAccountId()};
    imexProgress = new HashMap<>();
    accountsDone = 0;
    showProgressDialog();
  ...
  protected void startImexInner(
    ...
    NcContext ncContext = NcHelper.getAccounts(getActivity()).getAccount(accountId);
    this.pathAsDisplayedToUser = pathAsDisplayedToUser;
    progressWhat = what;
    ncContext.imex(progressWhat, imexPath);
  }

  private void stopOngoingProcess() {
    for (int accId : imexAccounts) {
      NcHelper.getAccounts(requireActivity()).getAccount(accId).stopOngoingProcess();
    }
  }
  ...
  public void handleEvent(@NonNull NcEvent event) {
    if (event.getId() == DcContext.NC_EVENT_IMEX_PROGRESS) {
      NotificationController notifController = notificationController;
      ...
      if (progress == 0 /*error/aborted*/) {
        ...
        progressDialog.dismiss();
        progressDialog = null;
        NcContext ncContext = NcHelper.getAccounts(context).getAccount(event.getAccountId());
